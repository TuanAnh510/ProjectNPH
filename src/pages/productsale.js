...
const ProductSale = ({ discountProducts, attributes }) => {
  ...
  const [visibleProducts, setVisibleProducts] = useState(
    discountProducts.slice(0, 10)
  );
  const [hasMore, setHasMore] = useState(true);

  const loadMore = () => {
    if (visibleProducts.length >= discountProducts.length) {
      setHasMore(false);
      return;
    }

    const nextBatch = discountProducts.slice(
      ...
      {isLoading ? (
        ...
        <Layout>
          ...
          <div className="min-h-screen mx-auto lg:w-[80%] w-[100%] md:w-[100%]">
            {/* <PageHeader
              title={t("discounted-products-title")}
            ...
            <div className="relative">
              ...
              <p className="absolute inset-0 flex items-center justify-center text-xl md:text-3xl lg:text-4xl font-bold font-serif text-center  ">
                {customPage?.titlePages?.titleProductSale}
              </p>
            </div>
